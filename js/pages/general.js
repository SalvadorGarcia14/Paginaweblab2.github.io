...
function recuadroAgrupacionPolitica(valoresTotalizadosPositivos) {
    ...
    const numAgrupacionesAMostrar = Math.min(6, valoresTotalizadosPositivos ? valoresTotalizadosPositivos.length : 0);
    ...
    for (let index = 0; index < numAgrupacionesAMostrar; index++) {
        ...
        const porcentajeAgrupacion = document.createElement("p");
        porcentajeAgrupacion.textContent = `${agrupacion.votosPorcentaje.toFixed(2)}%`;

        const cantVotos = document.createElement("p");
        cantVotos.textContent = `${agrupacion.votos} VOTOS`;
        ...
        progressBar.style.width = `${agrupacion.votosPorcentaje.toFixed(2)}%`;

        const progressBarText = document.createElement("span");
        progressBarText.className = "progress-bar-text";
        progressBarText.textContent = `${agrupacion.votosPorcentaje.toFixed(2)}%`;

        // Agregar elementos al contenido del recuadro
        contenidoRecuadro.appendChild(porcentajeAgrupacion);
        contenidoRecuadro.appendChild(cantVotos);
        progressBar.appendChild(progressBarText);
        progressBarContainer.appendChild(progressBar);
        contenidoRecuadro.appendChild(progressBarContainer);
...
function recuadrosBarras(valoresTotalizadosPositivos) {
    ...
    for (let index = 0; index < numAgrupacionesAMostrar; index++) {
        ...
        bar.className = `bar partido${index + 1}`;
        bar.style = `--bar-value: ${agrupacion.votosPorcentaje.toFixed(2)}%;`;
        bar.dataset.name = agrupacion.nombreAgrupacion;
        bar.title = `${agrupacion.nombreAgrupacion}\n${agrupacion.votosPorcentaje.toFixed(2)}%`;
        chartWrap.appendChild(bar);
    }
    ...
    if (votosOtros > 0) {
        const barOtros = document.createElement("div");
        barOtros.className = "bar partido8";
        barOtros.style = `--bar-value: ${votosOtros.toFixed(2)}%;`;
        barOtros.dataset.name = "Otros";
        barOtros.title = `Otros\n${votosOtros.toFixed(2)}%`;
        chartWrap.appendChild(barOtros);
    }
}